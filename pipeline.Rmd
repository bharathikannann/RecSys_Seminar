...
def covert_rf_to_fm(choose_method:int, x_col_rf, X_train, genre_cols):
    ...
    * genre_cols - The total genre list. Used to group them in group shapes.
    ...
    for col in ['timestamp', 'release_date']:
        if col in x_col_rf:
            x_col_rf.remove(col)
    ...
    # Convert only the column present in the selected method
    for col in x_col_rf:
        # For all normal ohe columns
        if col in ['movie_id','user_id','fm_timestamp','fm_release_date','zip_code','fm_age','occupation','last_watched','sex']:
    ...
    # So these columns are processed separately. It is done at last to make any confusion :)
    # if both are present
    if 'mean_rate' and 'cum_mean_rate' in x_col_rf:
        # a is union, has both mean and cum because of regex
        # b is intersction and has only cum_mean_rating
        a = X_train.filter(regex='mean_rate', axis=1)
        b = X_train.filter(regex='cum_mean_rate', axis=1)
        # Negation of 'b' with 'a' gives only mean_rating and it is selected from union
    ...
    # If atleast 1 is present, No clash and there will be no clash
    else:
        for col in x_col_rf:
            if col in ['mean_rate','cum_mean_rate']:
                X_train_rf = pd.concat([X_train_rf, X_train[col]], axis = 1)
                meta_data_rf[col] = 1
...
> **Important - Need to change the name of the selected columns (sel_from_model_col_rf) for everymethod**
...
```{python}
X_train_sel_from_model.iloc[:,1680+240+1681+943:]
```

```{python}
# Timestamp and release date columns have extra 'fm' character before. 
# For conversion for rf columns to fm columns, names are changed
for col in ['timestamp', 'release_date']:
    if col in sel_from_model_col_rf:
        sel_from_model_col_rf.remove(col)
        sel_from_model_col_rf.append('fm_'+col)

# Adding movie_id and user_id columns if not present
for col in ['movie_id', 'user_id']:
    if col not in sel_from_model_col_rf:
        sel_from_model_col_rf.append(col)
```
...
```{python}
X_col_rf = sel_from_model_col_rf
```
...
```{python}
if 'mean_rate' and 'cum_mean_rate' in ['mean_rate','cum_mean_rate']:
    print("Hi")
```

```{python}
a = X_train.filter(regex='jvhj', axis=1)
```
