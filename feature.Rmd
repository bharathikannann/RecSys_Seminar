...
RFE_feature_importances = pd.DataFrame(
    {"feature": feature_names, "importance": rfe.feature_importances_}
)
```
...
RFE_final_dict = {
    "k": None,
    "columns": RFE_feature_importances['feature'].tolist(),
    "importances": RFE_feature_importances['importance'].tolist(),
}
```

```{python}
# list of column names
ridge = list(X.columns)

# extract the feature importance values
std = np.std([tree.feature_importances_ for tree in forest.estimators_], axis=0)
ridge_feature_importances = pd.DataFrame(
    {"feature": feature_names, "importance": forest.feature_importances_}
)
```
