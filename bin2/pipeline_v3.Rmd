...
```{python}
def covert_fs_to_fm_v2(cols, X_fm, genre_cols):
    ...
    * genre_cols - The total genre list. Used to group them in group shapes.
    ...
    X_train = pd.concat([X_fm.filter(regex = 'user_id'), X_fm.filter(regex = 'movie_id')], axis = 1)
    group_shapes['user_id'] = 943
    group_shapes['movie_id_id'] = 1681
    
    for col in cols:
        if col in ['timestamp','release_date','age','sex']:
            X_train = pd.concat([X_train, X_fm.filter(regex = col)], axis = 1)
            group_shapes[col] = 1

    for col in ['occupation','zip_code']:
        if list(filter(re.compile(".*" + col).match, cols)):
            group_shapes[col] = 0
            for col_inside in list(filter(re.compile(".*" + col).match, cols)):
                X_train = pd.concat([X_train, X_fm[col_inside]])
                group_shapes[col] = group_shapes[col] + 1

    for col in cols:
        if col in ['mean_rate','cum_mean_rate']:
            X_train = pd.concat([X_train, X_fm[col]], axis = 1)
            group_shapes[col] = 1
                
    if list(set(cols) & set(genre_cols)):
        group_shapes['genre'] = 0
        for col in list(set(cols) & set(genre_cols)):
            temp = X_fm.filter(regex=col, axis=1)
            X_train = pd.concat([X_train, temp], axis = 1)
            group_shapes['genre'] = group_shapes['genre'] + 1 
            
    return X_train, group_shapes
```
...
def covert_fs_to_fm(cols, X_fm, genre_cols):
    ...
    * genre_cols - The total genre list. Used to group them in group shapes.
    ...
    X_train_temp = pd.concat([X_fm.filter(regex = 'user_id'), X_fm.filter(regex = 'movie_id')], axis = 1)
    group_shapes['user_id'] = 943
    group_shapes['movie_id_id'] = 1681
    ...
    for i in ['timestamp','release_date','age','sex']:
        if len(X_temp.filter(regex = i).columns):
            X_train_temp = pd.concat([X_train_temp, X_fm.filter(regex = i)], axis = 1)
            group_shapes[i] = len(X_temp.filter(regex = i).columns)
    
    if 'cum_mean_rate' in cols:
        X_train_temp['cum_mean_rate'] = X_fm['cum_mean_rate'] 
        group_shapes['cum_mean_rate'] = 1
    if 'mean_rate' in cols:
        X_train_temp['mean_rate'] = X_fm['mean_rate']
        group_shapes['mean_rate'] = 1
    ...
    occupation = list(filter(re.compile(".*occupation").match, cols))
    if occupation:
        X_train_temp = pd.concat([X_train_temp, X_fm[occupation]], axis = 1)
        group_shapes['occupation'] = len(occupation)
        
    zip_code = list(filter(re.compile(".*zip_code").match, cols))
    if zip_code:
        X_train_temp = pd.concat([X_train_temp, X_fm[zip_code]], axis = 1)
        group_shapes['zip_code'] = len(zip_code)
        
    group_shapes_list = [j for i,j in group_shapes.items()]
    return X_train_temp, group_shapes, group_shapes_list
```
...
```{python}
X_train_temp, group_shapes_dict, group_shapes = covert_fs_to_fm(sel_from_model_col_rf[:65], X_train, genre_cols)
```
...
for k in [0,5,10,15,20,25,30,35,40,45,50,55,60,64]:

    X_train_temp, group_shapes_dict, group_shapes = covert_fs_to_fm(sel_from_model_col_rf[:k], X_train, genre_cols)
    X_test_temp, group_shapes_dict, group_shapes = covert_fs_to_fm(sel_from_model_col_rf[:k], X_test, genre_cols)


    fm = MyFMRegressor(rank=10, random_seed = 332)
    fm.fit(X_train_temp, y_train, n_iter=200, n_kept_samples=200, group_shapes = group_shapes)

    prediction = fm.predict(X_test_temp)
    rmse = ((y_test - prediction) ** 2).mean() ** .5
    mae = np.abs(y_test - prediction).mean()
    print(f'rmse={rmse}, mae={mae}')
...
if 'cum_mean_rate' in sel_from_model_col_rf:
    sel_from_model_col_rf.remove('cum_mean_rate')
if 'mean_rate' in sel_from_model_col_rf:
    sel_from_model_col_rf.remove('mean_rate')
...
for k in [0,50,100,150,200,250,300,350,400,450,500, len(sel_from_model_col_rf)-1]:

    X_train_temp = pd.concat([X_train_temp, X_train[sel_from_model_col_rf[:k]]], axis = 1)
    X_test_temp = pd.concat([X_test_temp, X_test[sel_from_model_col_rf[:k]]], axis = 1)

    fm = MyFMRegressor(rank=10, random_seed = 332)
    fm.fit(X_train_temp, y_train, n_iter=200, n_kept_samples=200)
...
```{python}
temp = pd.read_csv("feature_selection/final_results/random_forest_V2.csv", header = None)
temp2 = pd.read_csv("feature_selection/final_results/extra_tree_classifier.csv", header = None)
```
...
plt.ylim(0.85,0.93)
plt.xlim(550,-50)
plt.plot([0,50,100,150,200,250,300,350,400,450,500, 524],rmse_results[1:], color = 'b', label = "Random Forest", linestyle = '--', marker = '*')
plt.plot([0,50,100,150,200,250,300,350,400,450,500,524],temp.values[1:], color = 'r', label = "Extra Tree classifier", linestyle = '--', marker = '*')
plt.plot([0,50,100,150,200,250,300,350,400,450,500,524],temp2.values[1:], color = 'y', label = "Permutation Importance", linestyle = '--', marker = '*')
...
```{python}
np.savetxt("feature_selection/final_results/permutation_importance.csv", 
           rmse_results,
           delimiter =", ", 
           fmt ='% s')
```
