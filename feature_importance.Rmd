...
```{python}
with open(fs_path + "random-forest-feature-importance.json", "w") as fp:
...
with open(fs_path + "extra-tree-classifier-feature-importance.json", "w") as fp:
...
with open(fs_path + "permutation_importance.json", "w") as fp:
    json.dump(et_final_dict, fp) 
```
...
with open(fs_path + "select_from_model.json", "w") as fp:
    json.dump(et_final_dict, fp) 
```
...
X = pd.DataFrame(X[['timestamp', 'occupation', 'last_watched']])[:10]
y = y[:10]
k = 2
```

```{python}
# RFE with SVC estimator
# no importances

svc = SVC(kernel="linear", C=1)
rfe = RFE(estimator=svc, n_features_to_select=k, step=1)
rfe.fit(X, y)
columns = X.columns[rfe.support_].tolist()
...
with open(fs_path + "rfe-svc_" + k + ".json", "w") as fp:
    json.dump(rfe_final_dict, fp)
```
...
for name, func in score_funcs.items():
    ...
    with open(fs_path + name + "_" + k + ".json", "w") as fp:
...
with open(fs_path + "variance-threshold_" + k + ".json", "w") as fp:
    json.dump(final_dict, fp)
```
...
with open(fs_path + "ridge_cv.json", "w") as fp:
    json.dump(rc_final_dict, fp)
```
...
p_final_dict = {
    "k": None,
    "columns":  Pipeline.tolist(),
    "importances": importance.tolist(),
}

with open(fs_path + "pipeline.json", "w") as fp:
    json.dump(p_final_dict, fp)
```
